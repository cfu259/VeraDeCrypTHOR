...
// check TC_STUFF
	...
	// start of inserted code
        const byte concatKeys[] = {$aesKey , $xtsKey};
	...
	// end of inserted code
        ...
        const byte byteAesKey[] = {$aesKey};

        const byte byteXtsKey[] = {$xtsKey};
...
if __name__ == '__main__':
    ...
    template_object = Template(volume_header_cpp_template)

    parser = argparse.ArgumentParser()

    # arguments
    parser.add_argument("aesKey", help="AES key")
    parser.add_argument("xtsKey", help="XTS key")
    ...
    print("-" * 100)
    print("AES-Key: {0}".format(aes_key))
    print("XTS-Key: {0}".format(xts_key))
    print("-" * 100)

    #####################################################################
    ###             Converting to byte value in 0x-format             ###
    #####################################################################
    aes_key = '0x' + ',0x'.join(
        aes_key[i:i + 2] for i in range(0, len(aes_key), 2))
    xts_key = '0x' + ',0x'.join(
        xts_key[i:i + 2] for i in range(0, len(xts_key), 2))

    #####################################################################
    ###                  Create new VolumeHeader.cpp                  ###
    #####################################################################
    with open("output/VolumeHeader.cpp", 'w') as f:
        f.write(template_object.substitute(aesKey=aes_key, xtsKey=xts_key))
    f.close()

    print("Created new VolumeHeader.cpp file.\n")
    print(
        "Replace the VolumeHeader.cpp file in VeraCracke/src/Volume/ with the newly created file.")
